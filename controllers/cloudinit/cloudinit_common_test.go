...
func TestCloudConfigInput(t *testing.T) {
	...
	t.Run("SnapstoreHTTPProxy", func(t *testing.T) {
		for _, tc := range []struct {
			name            string
			makeCloudConfig func() (*cloudinit.CloudConfig, error)
		}{
			{
				...
				makeCloudConfig: func() (*cloudinit.CloudConfig, error) {
					return cloudinit.NewInitControlPlane(&cloudinit.ControlPlaneInitInput{
						...
						Token:               strings.Repeat("a", 32),
						TokenTTL:            100,
						SnapstoreHTTPProxy:  "http://http.proxy:3128",
						SnapstoreHTTPSProxy: "http://https.proxy:3128",
					})
				},
			},
			{
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				g.Expect(c.RunCommands).To(ContainElement(`/capi-scripts/00-configure-snapstore-http-proxy.sh "http://http.proxy:3128" "http://https.proxy:3128"`))
			})
		}
	})
}
